...
pub enum EarlyExitLabel {
    // Value is the token we received from a landingpad/cleanuppad instruction
    // which we need to resume unwinding with.
    UnwindExit(ValueRef),
    ReturnExit,
...
impl<'blk, 'tcx> CleanupMethods<'blk, 'tcx> for FunctionContext<'blk, 'tcx> {
    ...
    /// will run the panic cleanups and eventually invoke the LLVM `Resume`
    /// instruction.
...
impl<'blk, 'tcx> CleanupHelperMethods<'blk, 'tcx> for FunctionContext<'blk, 'tcx> {
    ...
    fn trans_cleanups_to_exit_scope(&'blk self,
                ...
                match label {
                    UnwindExit(lp) => {
                        ...
                        if base::wants_msvc_seh(self.ccx.sess()) {
                            lpad = Some(LandingPad::msvc(lp));
                            let prev_bcx = self.new_block(lpad.clone(),
                                                          "resume", None);
                            build::CleanupRet(prev_bcx, lp, None);
        ...
        while let Some(mut scope) = popped_scopes.pop() {
            if !scope.cleanups.is_empty() {
                ...
                let lpad = if label.is_unwind() {lpad.clone()} else {None};
                let bcx_in = self.new_block(lpad, &name[..], None);
    ...
    fn get_or_create_landing_pad(&'blk self) -> BasicBlockRef {
        ...
        let llretval = if base::wants_msvc_seh(self.ccx.sess()) {
        ...
        let cleanup_llbb = self.trans_cleanups_to_exit_scope(UnwindExit(llretval));
